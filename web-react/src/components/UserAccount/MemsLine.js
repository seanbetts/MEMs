...
const MemsLine = () => {
    ...
    const orderedMems = _.orderBy(data.Mem, ['date.year', 'date.month'], ['asc', 'asc']);
    const memsByMonth = _.groupBy(orderedMems, item => `${item.date.year}-${item.date.month}`)
            ...
            return (
                <ThemeProvider theme={Theme}>
                    ...
                    <div className={classes.timelineContainer}>
                        <div key={i} className={classes.monthlyTitle}>{(dateformat(month, 'mmmm yyyy')).toLowerCase()}</div>
